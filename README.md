...
```json
{
  ...
  "rate_note": "Modelled, not observed: no per-site enrolment is published. Largely set by site count (slope near -1), so compare like-sized trials.",
...
|-------|--------------|-------|----|--------------------|------|
| P1HV | 3.26 mo | +0.27 | 0.331 | 0.79 (0.85) | pass |
| P1 | 7.72 mo | +0.35 | 0.648 | 0.79 (0.80) | pass |
| P2 | 9.54 mo | +0.22 | 0.425 | 0.79 (0.80) | pass |
| P3 | 10.03 mo | +0.23 | 0.399 | 0.78 (0.80) | pass |

Fold: horizon fold: train on trials starting before 2018, test on 2018–2020 starts, which have had 5.4–8.6 years to finish against a corpus whose p95 duration is 5.9. Rows P1HV=370, P1=376, P2=377, P3=378 of `experiments/ledger.jsonl`. Regenerate: `python -m experiments.publish_metrics`. Every row measures the shipped configuration: IPCW censoring weights applied, as in `trainer.train_phase`.
...
Reproduce: `python -m experiments.run --config two_stage_l2_ipcw_total --phases P1,P2,P3 --split horizon`
and `--config two_stage_l2_cov85_ipcw_total --phases P1HV`; for the served rate the
`derived_rate_*_ipcw_total` configs with `--target recruitment_rate`; then
`python -m experiments.publish_metrics` to refresh every table from the ledger. The `_ipcw` configs
build their censoring frame with `trainer.build_censoring_frame`, the same function training uses;
until 2026-08-30 the eval configs passed no frame, so the table measured an unweighted model that
was not the one serving. `_total` names the IPCW scope that ships (`trainer.IPCW_SCOPE`): one weight
per trial from its total duration, applied to both stages. The earlier scope weighted the enrolment
stage alone, looked up at the enrolment window against a KM over total duration, and left follow-up
unweighted; fixing it moved R² +0.004 / +0.002 / +0.007 on P1 / P2 / P3 and was level on P1HV.
...
|-------|------------------------------------|--------------|-------|--------------------|------|
| P1HV | 20.90 | 23.22 | +0.10 | 0.85 (0.85) | pass |
| P1 | 11.74 | 14.38 | +0.18 | 0.84 (0.80) | pass |
| P2 | 3.71 | 5.37 | +0.31 | 0.81 (0.80) | pass |
| P3 | 10.57 | 15.10 | +0.30 | 0.82 (0.80) | pass |

Fold: horizon fold: train on trials starting before 2018, test on 2018–2020 starts, which have had 5.4–8.6 years to finish against a corpus whose p95 duration is 5.9. Rows P1HV=372, P1=382, P2=383, P3=384 of `experiments/ledger.jsonl`. Regenerate: `python -m experiments.publish_metrics`.
...
The app is a single `uvicorn` process; no separate frontend server is needed.
