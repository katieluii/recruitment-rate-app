...
**therapeutic area** (Oncology, CNS, Cardiovascular, …), and the app returns:
...
falls back to the median for that **therapeutic area**, so asking for "a Phase 3
...
```json
{
  "predicted_months": 28.7,
  "lower_months": 13.6,
  "upper_months": 43.7,
  "predicted_days": 872.5,
  "confidence_pct": 80,
  "recruitment_rate": 0.312,
  "recruitment_rate_lower": 0.197,
  "recruitment_rate_upper": 1.326,
  "estimated_recruitment_months": 25.6,
  "recruitment_rate_target_quality": "B",
  "recruitment_rate_n_train": 1280,
  "enrollment_used": 120.0,
  "num_sites_used": 15.0,
  "endpoint_archetypes_used": ["SURVIVAL"],
  "rate_note": "Estimated from completed-trial record histories …",
...
Duration and recruitment speed are separate processes. P1/P2/P3 therefore use
independent duration and recruitment-rate heads; P1HV retains duration only.
Total duration fuses recruitment with follow-up, and follow-up is what makes an
oncology Phase 3 run materially longer than a short-readout design.

| Head | Target | Transform |
|------|--------|-----------|
| **A: duration** | days, start → primary completion | `log1p` |
| **B: recruitment rate** | actual enrolment / (initiated sites × recorded recruiting months) | `log` |
...
That fold cannot contain a long trial, because the corpus is completed trials only and a trial
starting in 2022 that runs six years is not in it, so it rewarded anything that predicted
...
Reproduce: `python -m experiments.run --config hybrid_rf_refit_ipcw_total --split horizon`
for the duration model and `--config derived_rate_hybrid_refit_ipcw_total --target recruitment_rate`
for the served rate, then `python -m experiments.publish_metrics` to refresh every table from the
ledger. The shipped model (v5, 2026-08-31) is the v1 random forest's point estimate inside the
two-stage model's calibrated band, with the recruiting/follow-up split rescaled to the forest
total; `docs/VERSION_HISTORY.md` records how it was chosen. Every `_ipcw` config builds its
censoring frame with `trainer.build_censoring_frame`, the same function training uses, and
`_total` names the IPCW scope that ships: one weight per trial from its total duration, applied
to both stages.
See `experiments/README.md` for the protocol and the v1 comparison.

Recruitment rate is now fitted directly to ClinicalTrials.gov record histories. The
target is final **ACTUAL** enrolment divided by initiated centres and the recorded
recruiting interval. It is independent of the duration head; changing the planning
sliders applies `participants / (centres × predicted rate)` without rerunning either
model.

| Phase | usable history targets | temporal test | MAE | median factor error | 80% coverage | skill vs area median |
|---|---:|---:|---:|---:|---:|---:|
| P1 | 1,107 | 285 | 1.7885 | 1.541× | 0.818 | +0.285 |
| P2 | 1,280 | 354 | 3.1387 | 1.547× | 0.884 | +0.251 |
| P3 | 1,248 | 299 | 3.7444 | 1.590× | 0.829 | +0.462 |

The temporal test window is 2021–2022, trained on starts before 2021. P1HV has
no released rate head. Tier B assumes listed/initiated centres had the full
recorded recruiting interval; it is a planning estimate, not observed performance
for every centre. See `docs/HISTORY_RATE_VALIDATION_2026-09-03.md`.

The block below records the superseded duration-derived proxy and old standalone
cross-check for reproducibility. Neither is returned as the current rate:
...
* **The rate target is trial-level, not observed centre performance.** Tier B
  divides final actual enrolment by initiated/listed centres and the recorded
  recruiting interval. It assumes those centres had the full interval; staggered
  activation is only available for the smaller Tier A validation subset.
...
The app is a single `uvicorn` process, so no separate frontend server is needed.
